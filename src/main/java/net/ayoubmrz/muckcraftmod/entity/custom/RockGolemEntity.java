...
public class RockGolemEntity extends HostileEntity implements GeoEntity {

    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final TrackedData<Boolean> IS_SHOOTING = DataTracker.registerData(RockGolemEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    private PlayState shootPredicate(AnimationState<RockGolemEntity> event) {

        if (this.isShooting()) {
            event.getController().forceAnimationReset();
            event.getController().setAnimation(
                    RawAnimation.begin().then("animation.rock_golem.shoot", Animation.LoopType.PLAY_ONCE)
            );
            setShooting(false);
    ...
    private PlayState attackPredicate(AnimationState<RockGolemEntity> event) {
        if (this.handSwinging) {
            event.getController().forceAnimationReset();
            event.getController().setAnimation(
                    RawAnimation.begin().then("animation.rock_golem.attack", Animation.LoopType.PLAY_ONCE)
            );
    ...
    private PlayState predicate(AnimationState<RockGolemEntity> animationState) {
        ...
        controller.setAnimation(RawAnimation.begin().then("animation.rock_golem.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
